...
fn main() {
    ...
    points = points.iter().map(|p| {
        let new_row = p.row + empty_rows.iter().filter(|r| **r < p.row).count();
        let new_column = p.column + empty_columns.iter().filter(|c| **c < p.column).count();
        Point { column: new_column, row: new_row }
    }).collect::<Vec<Point>>();
