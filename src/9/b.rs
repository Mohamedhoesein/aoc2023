...
fn main() {
    ...
    for line in fs::read_to_string(file_name).unwrap().lines() {
        ...
        let mut temp = current_changes.clone().iter().zip(current_changes.iter().skip(1)).map(|(x, y)| (x-y)).collect::<Vec<i64>>();
        all_changes.push(temp.clone());
        while temp.clone().iter().any(|x| *x != 0) {
            temp = temp.iter().zip(temp.iter().skip(1)).map(|(x, y)| (x-y)).collect::<Vec<i64>>();
            all_changes.push(temp.clone());
        }
